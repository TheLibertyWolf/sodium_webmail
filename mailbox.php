...
<div class="mail-page-shell">
    <div class="mail-toolbar">
        ...
        <form method="get" action="/mailbox.php" class="input-group mail-search">
            ...
            <input type="hidden" name="account_id" value="<?= (int)$account['id'] ?>"><input type="hidden" name="folder" value="<?= e($folderKey) ?>"><input type="hidden" name="status" value="<?= e($statusFilter) ?>"><input type="hidden" name="tag" value="<?= e($tagFilter) ?>"><input type="hidden" name="limit" value="<?= $messageLimit ?>">
            ...
            <?php if($searchQuery!==''): ?><a class="btn btn-outline-secondary" href="<?= e('/mailbox.php?account_id='.(int)$account['id'].'&folder='.rawurlencode($folderKey).'&status='.rawurlencode($statusFilter).'&tag='.rawurlencode($tagFilter).'&limit='.$messageLimit.'&scope='.rawurlencode($searchScope)) ?>" title="Effacer la recherche"><i class="bi bi-x-lg"></i></a><?php endif; ?>
        ...
        <?php if($availableTags): ?><select class="form-select form-select-sm tag-filter" onchange="location.href=this.value"><option value="<?= e($baseFolderUrl.'&status='.$statusFilter) ?>">Tous les tags</option><?php foreach($availableTags as $tag): ?><option value="<?= e($baseFolderUrl.'&status='.$statusFilter.'&tag='.rawurlencode((string)$tag['shared_key'])) ?>" <?= $tagFilter===(string)$tag['shared_key']?'selected':'' ?>><?= e($tag['name']) ?></option><?php endforeach; ?></select><?php endif; ?>
        <label class="d-flex align-items-center gap-2 ms-auto small text-muted">Afficher <select class="form-select form-select-sm w-auto" onchange="location.href=this.value"><?php foreach([15,25,50,100] as $limitOption): ?><option value="<?= e('/mailbox.php?account_id='.(int)$account['id'].'&folder='.rawurlencode($folderKey).'&status='.$statusFilter.'&tag='.rawurlencode($tagFilter).'&limit='.$limitOption.'&scope='.rawurlencode($searchScope).($searchQuery!==''?'&q='.rawurlencode($searchQuery):'').($deepSearch?'&deep=1':'')) ?>" <?= $messageLimit===$limitOption?'selected':'' ?>><?= $limitOption ?></option><?php endforeach; ?></select></label>
    </div>
    ...
    <?php else: ?><form method="post" action="/bulk-action.php" class="bulk-mail-form">
        ...
        <div class="bulk-actions">
            ...
            <div class="bulk-buttons">
                ...
                <?php if($availableTags): ?><div class="input-group input-group-sm tag-action-group"><select class="form-select" name="tag_shared_key"><option value="">Ajouter un tag…</option><?php foreach($availableTags as $tag): ?><option value="<?= e($tag['shared_key']) ?>"><?= e($tag['name']) ?></option><?php endforeach; ?></select><button class="btn btn-outline-secondary" name="bulk_action" value="tag"><i class="bi bi-tag"></i></button></div><?php endif; ?>
        ...
        ?>
            <div class="message-row <?= $message['unread'] ? 'unread' : '' ?>" data-message-row data-account="<?= (int)$account['id'] ?>" data-folder="<?= e($folderKey) ?>" data-uid="<?= (int)$message['uid'] ?>" data-reply-all="<?= sodium_message_has_reply_all($message,$account)?'1':'0' ?>">
                <span class="message-select-tools"><span class="message-drag-handle" title="Déplacer ce message" aria-hidden="true"></span><input class="form-check-input message-checkbox" type="checkbox" name="messages[]" value="<?= e($selection) ?>"><button class="message-star <?= $message['flagged']?'is-flagged':'' ?>" type="button" data-star-toggle title="<?= $message['flagged']?'Retirer des messages marqués':'Ajouter aux messages marqués' ?>"><i class="bi bi-star<?= $message['flagged']?'-fill':'' ?>"></i></button></span>
