...
if($tagFilter===''&&(int)($_GET['tag_id']??0)>0){$legacyTag=$pdo->prepare('SELECT shared_key FROM sodium_tags WHERE id=?');$legacyTag->execute([(int)$_GET['tag_id']]);$tagFilter=(string)($legacyTag->fetchColumn()?:'');}
...
$unifiedUrl=static function(array $overrides=[])use($statusFilter,$tagFilter,$messageLimit,$searchQuery,$searchScope):string{
    $params=['status'=>$statusFilter,'tag'=>$tagFilter,'limit'=>$messageLimit,'q'=>$searchQuery,'scope'=>$searchScope];
    foreach($overrides as $key=>$value)$params[$key]=$value;
    $params=array_filter($params,static fn($value,$key)=>!($key==='status'&&$value==='all')&&!($key==='tag'&&$value==='')&&!($key==='page'&&(int)$value<=1),ARRAY_FILTER_USE_BOTH);
...
<div class="mail-page-shell">
    <div class="mail-toolbar">
        ...
        <form method="get" action="/index.php" class="input-group mail-search">
            ...
            <input type="hidden" name="status" value="<?= e($statusFilter) ?>"><input type="hidden" name="tag" value="<?= e($tagFilter) ?>"><input type="hidden" name="limit" value="<?= $messageLimit ?>">
        ...
        <?php if($availableTags): ?><select class="form-select form-select-sm tag-filter" onchange="location.href=this.value"><option value="<?= e($unifiedUrl(['tag'=>'','page'=>1])) ?>">Tous les tags</option><?php foreach($availableTags as $tag): ?><option value="<?= e($unifiedUrl(['tag'=>(string)$tag['shared_key'],'page'=>1])) ?>" <?= $tagFilter===(string)$tag['shared_key']?'selected':'' ?>><?= e($tag['name']) ?></option><?php endforeach; ?></select><?php endif; ?>
