...
<div class="message-row unified <?= $message['unread']?'unread':'' ?>" data-message-row data-account="<?= (int)$account['id'] ?>" data-folder="<?= e($message['folder']) ?>" data-uid="<?= (int)$message['uid'] ?>" data-reply-all="<?= sodium_message_has_reply_all($message,$account)?'1':'0' ?>">
<span class="message-select-tools"><span class="message-drag-handle" title="Déplacer ce message" aria-hidden="true"></span><input class="form-check-input message-checkbox" type="checkbox" value="<?= e($selection) ?>"><button class="message-star is-flagged" type="button" data-star-toggle title="Retirer des messages marqués"><i class="bi bi-star-fill"></i></button></span>
