...
$root=__DIR__;$localFile=$root.'/config.local.php';$installedFile=$root.'/.installed';$installer=str_starts_with((string)($_SERVER['SCRIPT_NAME']??''),'/install/');
if(!is_file($localFile)||!is_file($installedFile)){if(!$installer&&PHP_SAPI!=='cli'){header('Location: /install/');exit;}return;}
$local=require $localFile;
foreach(['db_host','db_name','db_user','db_pass','base_url','turnstile_site_key','turnstile_secret_key'] as $key)if(!array_key_exists($key,$local))throw new RuntimeException('Configuration Sodium incomplète.');
define('DB_HOST',(string)$local['db_host']);define('DB_NAME',(string)$local['db_name']);define('DB_USER',(string)$local['db_user']);define('DB_PASS',(string)$local['db_pass']);define('DB_CHARSET','utf8mb4');
...
define('SODIUM_DEFAULT_LOCALE',(string)($local['default_language']??'fr'));
...
header('Strict-Transport-Security: max-age=31536000; includeSubDomains');header('X-Frame-Options: SAMEORIGIN');header('X-Content-Type-Options: nosniff');header('Referrer-Policy: strict-origin-when-cross-origin');header('Permissions-Policy: camera=(), microphone=(), geolocation=(), payment=(), usb=()');header("Content-Security-Policy: default-src 'self'; base-uri 'self'; form-action 'self'; frame-ancestors 'self'; object-src 'none'; script-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net https://challenges.cloudflare.com; style-src 'self' 'unsafe-inline' https://cdn.jsdelivr.net; font-src 'self' https://cdn.jsdelivr.net data:; img-src 'self' data: blob: https:; connect-src 'self'; frame-src 'self' blob: https://challenges.cloudflare.com");
...
require_once __DIR__.'/includes/platform/functions.php';require_once __DIR__.'/includes/i18n.php';require_once __DIR__.'/includes/platform/mail-lib.php';require_once __DIR__.'/includes/sodium.php';require_once __DIR__.'/includes/update.php';sodium_ensure_schema();
