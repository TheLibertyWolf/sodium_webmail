...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if($action==='save_user'){
        ...
        if($userId){$params=[$username,$first,$last,$email,$professional?:null,$status,$status==='active'?1:0,$twofaRequired];$sql='UPDATE users SET username=?,first_name=?,last_name=?,email=?,professional_email=?,account_status=?,is_active=?,twofa_required=?';if($password!==''){if(strlen($password)<12){flash('danger','Le mot de passe doit contenir au moins 12 caractères.');redirect('/admin/users.php');}$sql.=',password_hash=?';$params[]=password_hash($password,PASSWORD_DEFAULT);}$sql.=',updated_at=NOW() WHERE id=?';$params[]=$userId;$pdo->prepare($sql)->execute($params);if($status!=='active')clear_remember_cookie($userId,true);flash('success','Utilisateur modifié.');}
        else{$pdo->prepare("INSERT INTO users(username,password_hash,first_name,last_name,email,professional_email,role,account_status,is_active,theme,twofa_required) VALUES(?,?,?,?,?,?,'user',?,?,?,?)")->execute([$username,password_hash($password,PASSWORD_DEFAULT),$first,$last,$email,$professional?:null,$status,$status==='active'?1:0,'dark',$twofaRequired]);$newId=(int)$pdo->lastInsertId();$pdo->prepare("INSERT IGNORE INTO sodium_user_aptitudes(user_id,label) VALUES(?,'sodium_accounts_view')")->execute([$newId]);flash('success','Utilisateur créé.');}
...
<?php $editableUsers=array_merge([['id'=>0,'username'=>'','first_name'=>'','last_name'=>'','email'=>'','professional_email'=>'','account_status'=>'active','twofa_required'=>0]],$users);foreach($editableUsers as $editable):$editId=(int)$editable['id'];?>
