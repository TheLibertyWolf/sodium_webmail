...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if($action==='save_user'){
        $userId=(int)($_POST['user_id']??0);$username=trim((string)($_POST['username']??''));$first=trim((string)($_POST['first_name']??''));$last=trim((string)($_POST['last_name']??''));$email=trim((string)($_POST['email']??''));$professional=trim((string)($_POST['professional_email']??''));$status=in_array($_POST['account_status']??'active',['active','inactive','disabled'],true)?(string)$_POST['account_status']:'active';$password=(string)($_POST['password']??'');
        ...
        if($userId){$params=[$username,$first,$last,$email,$professional?:null,$status,$status==='active'?1:0];$sql='UPDATE users SET username=?,first_name=?,last_name=?,email=?,professional_email=?,account_status=?,is_active=?';if($password!==''){if(strlen($password)<12){flash('danger','Le mot de passe doit contenir au moins 12 caractères.');redirect('/admin/users.php');}$sql.=',password_hash=?';$params[]=password_hash($password,PASSWORD_DEFAULT);}$sql.=',updated_at=NOW() WHERE id=?';$params[]=$userId;$pdo->prepare($sql)->execute($params);if($status!=='active')clear_remember_cookie($userId,true);flash('success','Utilisateur modifié.');}
        else{$pdo->prepare("INSERT INTO users(username,password_hash,first_name,last_name,email,professional_email,role,account_status,is_active,theme) VALUES(?,?,?,?,?,?,'user',?,?,?)")->execute([$username,password_hash($password,PASSWORD_DEFAULT),$first,$last,$email,$professional?:null,$status,$status==='active'?1:0,'dark']);$newId=(int)$pdo->lastInsertId();$pdo->prepare("INSERT IGNORE INTO sodium_user_aptitudes(user_id,label) VALUES(?,'sodium_accounts_view')")->execute([$newId]);flash('success','Utilisateur créé.');}
        ...
        <div class="modal-dialog modal-xl modal-dialog-scrollable"><div class="modal-content"><form method="post" class="sodium-aptitudes-form">
            ...
            <div class="modal-body">
                ...
                <div class="aptitude-full-card mb-3">
                    <label class="form-check mb-0"><input class="form-check-input full-access-toggle" type="checkbox" name="aptitudes[sodium_full_access]" value="1" <?= $hasFullAccess?'checked':'' ?> <?= $isProtectedAdministrator?'disabled':'' ?>><span class="form-check-label"><span class="aptitude-card-icon"><i class="bi bi-shield-check"></i></span><span><strong>Full accès</strong><small><?= $isProtectedAdministrator?'Accès permanent de l’administrateur principal SQL 1 — cette aptitude ne peut pas être retirée.':'Accès à toutes les fonctions Sodium et à la gestion des utilisateurs.' ?></small></span></span></label>
                </div>
