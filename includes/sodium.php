...
function sodium_ensure_schema(): void
{
    ...
    sodium_ensure_column('users','language',"VARCHAR(5) NOT NULL DEFAULT 'fr'");
...
function sodium_instance_settings():array{global $pdo;sodium_ensure_schema();$defaults=['instance_name'=>'Sodium','organization_name'=>'','support_email'=>'','system_mail_account_id'=>'0','system_sender_name'=>'Sodium','system_mail_transport'=>'php','system_brevo_api_cipher'=>'','system_brevo_from_email'=>'','timezone'=>'Europe/Paris','cron_last_run_at'=>'','cron_last_status'=>'never','dependency_source'=>'local','turnstile_enabled'=>(defined('TURNSTILE_ENABLED')&&TURNSTILE_ENABLED?'1':'0'),'turnstile_site_key'=>(defined('TURNSTILE_SITE_KEY')?TURNSTILE_SITE_KEY:''),'turnstile_secret_cipher'=>''];$rows=$pdo->query('SELECT setting_key,setting_value FROM sodium_instance_settings')->fetchAll(PDO::FETCH_KEY_PAIR);return array_merge($defaults,$rows?:[]);}
...
function sodium_dependency_source():string{$source=(string)(sodium_instance_settings()['dependency_source']??'local');return in_array($source,['local','remote'],true)?$source:'local';}
